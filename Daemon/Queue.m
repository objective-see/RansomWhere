...
-(BOOL)shouldFullyProcess:(Event*)event
{
    ...
    if( (YES == event.process.wasDisallowed) ||
        (YES == [event.process.untrustedAncestor wasDisallowed]) )
    {
        //dbg msg
        #ifdef DEBUG
    ...
    //dbg msg
    #ifdef DEBUG
    logMsg(LOG_DEBUG, @"PASSED CHECK 5: is from non-disallowed process");
    #endif
...
-(void)processEvent:(Event*)event
{
    ...
    //dbg msg
    #ifdef DEBUG
    logMsg(LOG_DEBUG, @"PASSED CHECK 7: is encrypted");
    #endif
    ...
    //when ancestor that hit the limit, suspend it & kids
    ...
    //alert user & set 'wasReported' flag
...
-(CFOptionFlags)alertUser:(Event*)event
{
    ...
    //set flag here
    // ->yah, it's before the call, but the call blocks so do it now
    process.wasReported = YES;
