...
-(void)processEvent:(Event*)event
{
    ...
    if(YES == event.binary.isWhiteListed)
    {
        //dbg msg
        #ifdef DEBUG
    ...
    if( (YES == event.binary.isApple) &&
        (YES != event.binary.isGrayListed) )
    {
        //dbg msg
        #ifdef DEBUG
        logMsg(LOG_DEBUG, @"ignoring: is Apple binary (that's also not graylisted)");
        #endif
    ...
    //dbg msg
    #ifdef DEBUG
    logMsg(LOG_DEBUG, @"1) is not Apple binary (or is, but is graylist)");
    #endif
    ...
    //dbg msg
    #ifdef DEBUG
    logMsg(LOG_DEBUG, @"3) is not installed/baselined app");
    #endif
    ...
    //dbg msg(s)
    // ->always show
    syslog(LOG_ERR, "%s", [NSString stringWithFormat:@"OBJECTIVE-SEE RANSOMWHERE?: %@ is quickly creating encrypted files", event.binary.path].UTF8String);
